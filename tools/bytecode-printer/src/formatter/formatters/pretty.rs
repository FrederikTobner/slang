    ...
    fn format(&self, chunk: &Chunk, name: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        while offset < chunk.code.len() {
            ...
            match OpCode::from_int(instruction_byte) {
                Some(op) => {
                    let depth = scope_depth.get(&offset).unwrap_or(&0);
                    let (formatted, new_offset) = self.format_instruction(
                        chunk,
                        offset,
                        &op,
                        line,
                ...
                None => {
                    ...
                    result.push_str(&format!(
                        "{indent}{offset:04} {line:4} {} ({instruction_byte})\n",
                        "UNKNOWN".red().bold()
                    ));
                    offset += 1;
                }
            ...
            match OpCode::from_int(instruction_byte) {
                Some(OpCode::Jump) | Some(OpCode::JumpIfFalse) => {
                    if offset + 2 < chunk.code.len() {
                        let jump_offset = ((chunk.code[offset + 1] as usize) << 8)
                            | (chunk.code[offset + 2] as usize);
    ...
    fn format_instruction(
        ...
        match op {
            OpCode::Constant => {
                ...
                (
                    format!(
                        "{indent}{offset_str} {line_str} {} [{index}] '{value}'\n",
                        "CONST".green().bold(),
                        index = constant_index.to_string().cyan()
                    ),
                    offset + 2,
                )
            }
            OpCode::GetVariable | OpCode::SetVariable => {
                ...
                (
                    format!(
                        "{indent}{offset_str} {line_str} {} [{index}] '{name}'\n",
                        action.green().bold(),
                        index = var_index.to_string().cyan()
                    ),
                    offset + 2,
                )
            }
            OpCode::DefineFunction => {
                ...
                let fn_info = if fn_constant_index < chunk.constants.len() {
                    format!(
                        "'{constant}'",
                        constant = chunk.constants[fn_constant_index]
                    )
                } else {
                    "<??>".to_string()
                };
                (
                    format!(
                        "{indent}{offset_str} {line_str} {} [{var_idx}->{fn_idx}] {info} \n",
                        "DEF_FN".magenta().bold(),
                        var_idx = var_index.to_string().cyan(),
                        fn_idx = fn_constant_index.to_string().cyan(),
                        info = format!("{name} = {fn_info}").bright_white()
                    ),
                    offset + 3,
                )
            }
            OpCode::Call => {
                ...
                (
                    format!(
                        "{indent}{offset_str} {line_str} {} ({count} args)\n",
                        "CALL".blue().bold(),
                        count = arg_count.to_string().cyan()
                    ),
                    offset + 2,
                )
            }
            OpCode::JumpIfFalse | OpCode::Jump => {
                ...
                (
                    format!(
                        "{indent}{offset_str} {line_str} {} {jump_str} → {target:04}\n",
                        jump_type.red().bold(),
                        jump_str = format!("+{jump_offset}").yellow()
                    ),
                    offset + 3,
                )
            }
