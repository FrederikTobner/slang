...
impl super::super::BytecodeFormatter for DebugFormatter {
    fn format(&self, chunk: &Chunk, name: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        output.push_str(&format!("╭─ BYTECODE CHUNK: {name} ─╮\n", name = name.cyan().bold()));
        
        // Chunk statistics
        output.push_str(&format!("│ Code size: {size} bytes\n", size = chunk.code.len().to_string().yellow()));
        output.push_str(&format!("│ Constants: {constants}\n", constants = chunk.constants.len().to_string().green()));
        output.push_str(&format!("│ Identifiers: {identifiers}\n", identifiers = chunk.identifiers.len().to_string().blue()));
            ...
            if i % 16 == 15 || i == chunk.code.len() - 1 {
                ...
                for j in start..end {
                    ...
                    if (32..=126).contains(&byte) {
                        output.push_str(&format!("{ch}", ch = byte as char).bright_white().to_string());
                    } else {
                        output.push_str(&".".dimmed().to_string());
                    }
                }
        ...
        while offset < chunk.code.len() {
            ...
            output.push_str(&format!("│ {offset:04x}: {instruction_byte:02x} ").yellow().to_string());
        ...
        if !chunk.constants.is_empty() {
            ...
            for (i, constant) in chunk.constants.iter().enumerate() {
                output.push_str(&format!("│ {index:3}: {value}\n", 
