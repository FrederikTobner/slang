...
fn pretty_format_basic() {
    let (_temp_dir, file_path) =
        create_temp_slang_file(SAMPLE_SLANG_CODE).expect("Failed to create temp file");

    let result = parse_and_print_ast(file_path.to_str().unwrap(), AstFormat::Pretty);
...
fn json_format_basic() {
    let (_temp_dir, file_path) =
        create_temp_slang_file(SIMPLE_EXPRESSION).expect("Failed to create temp file");

    let result = parse_and_print_ast(file_path.to_str().unwrap(), AstFormat::Json);
...
fn compact_format_basic() {
    ...
    let result = parse_and_print_ast(file_path.to_str().unwrap(), AstFormat::Compact);
...
fn invalid_slang_syntax() {
    ...
    let (_temp_dir, file_path) =
        create_temp_slang_file(invalid_code).expect("Failed to create temp file");
...
#[test]
fn empty_file() {
    let (_temp_dir, file_path) = create_temp_slang_file("").expect("Failed to create temp file");
...
fn all_formats_with_complex_code() {
    let (_temp_dir, file_path) =
        create_temp_slang_file(COMPLEX_CODE).expect("Failed to create temp file");
...
fn file_extension_warning() {
    ...
    write!(temp_file, "{SIMPLE_EXPRESSION}").expect("Failed to write to temp file");

    let result = parse_and_print_ast(temp_file.path().to_str().unwrap(), AstFormat::Pretty);

    assert!(
        result.is_ok(),
        "Should parse file even without .sl extension"
    );
}
...
fn error_message_contains_relevant_info() {
    let (_temp_dir, file_path) =
        create_temp_slang_file("invalid syntax").expect("Failed to create temp file");
