...
fn end_to_end_compilation(bencher: Bencher) {
    bencher.bench(|| {
        ...
        for program in programs.iter() {
            ...
            let _ = black_box(ChainPipeline::full_compilation(program.source).compile_to_bytecode());
        }
    });
}
...
fn e2e_program_complexity(bencher: Bencher, complexity: usize) {
    ...
    bencher.bench(|| {
        ...
        let pipeline = ChainPipeline::full_compilation(&program.source)
            .with_file_name("test.sl".to_string());
        let _ = black_box(pipeline.execute());
    });
}
