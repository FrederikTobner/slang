...
use slang_compilation_pipeline::{ChainPipeline, result::CompilationResult as PipelineResult};
...
pub fn compile_to_bytecode(program: &str) -> Result<Chunk, String> {
    ...
    let result = ChainPipeline::full_compilation().execute(source_file.unwrap());
...
pub fn parse_only(program: &str) -> Result<Vec<Statement>, String> {
    let source_file = SlangSourceFile::new("benchmark.sl", program.to_string());
    let result = ChainPipeline::parsing_only().execute(source_file.unwrap());

    match result {
        PipelineResult::Success { output, .. } => Ok(output),
        PipelineResult::Failed { .. } => Err("AST compilation failed".to_string()),
    }
}
...
pub fn semantic_analysis_only(program: &str) -> Result<Vec<Statement>, String> {
    let source_file = SlangSourceFile::new("benchmark.sl", program.to_string());
    let result = ChainPipeline::ast_compilation().execute(source_file.unwrap());

    match result {
        PipelineResult::Success { output, .. } => Ok(output),
        PipelineResult::Failed { .. } => Err("Semantic analysis failed".to_string()),
    }
}
